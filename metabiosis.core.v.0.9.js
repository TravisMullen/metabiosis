...
var Metabiosis = mBss = (function() {

    var MBS = {}, // `MBS` Public Module namespace
    ...
    function processConfig(cfg) {
        if ( typeof(cfg.paths) === 'object') {
            __paths = cfg.paths;
            // MBS.log("we got paths!");
        } else {
            MBS.log("you need some action paths!");
            return false;
        }
        if ( typeof(cfg.paths) === 'object' ) {
            __itemKeys = Object.keys(cfg.items);
            __items = cfg.items;
            MBS.log("itemKeys: " + __itemKeys);
        } else {
            MBS.log("you need some action items!");
            return false;
        }

        MBS.log("valid config");

        // var p = processItems( __items, __itemKeys );
        // if (p) {
        //     MBS.log("items and actions are processed");
        //     return true;
        // } 
    ...
    function queueEvents( pathsForKey, key ) {
        ...
        for (var i = 0; i < pathsForKey.length; i++) {
            ...
            console.log("pathsForKey[i]",pathsForKey[i]);
            ...
            //     //     // if actions, 
            //     //     // grab the last event and add them to it before creating the next event in queue
            //     //     last = last.concat( actions.slice(0) );
            //     //     console.log("last",last);
            //     //     // clear actions
            //     //     actions = [];
            //     // }
            //     // // now add next not fibulated action 
            //     // // as own array (note etra `[]`)
            //     // events.push( [ pathsForKey[i] ] );
            // }
            else {
                events.push( actions.slice(0) );
                actions = [];
        ...
        // todo :: put these back in a hash
        // eventHash[key] = events.slice(0);
        eventHash[key] = events;
        console.log("kqueueEvents for key",key);
        console.log("eventHash[key]",eventHash[key]);
        // eventQueue.push(events); // queue up each set if event 
        // console.log("queueEvents // eventQueue",eventQueue);
    }
    ...
    //     } else {
    //      MBS.log("no action paths for this type of jawn");
    //      // return false;
    //     }
    //     return events;
    // }
    ...
    function queueHandler() {
        ...
        // get first
        rItems = __item[ __itemKeys[0] ].length,
...
// };
     ...
     * ============================================= */
    MBS.__development = true;
    MBS.__visualCues = true;
    ...
    //     click: clickSyth
    // };
    MBS.run = function(config) {
        var c = processConfig(config);
        if (c) {
            MBS.log("starting the show!");
            startEvents(c);
        }
    };
    ...
    MBS.log = function(message) {
        if (MBS.__development) {
            console.log(message);
            //     return true; // to check if message was logged
            // } else {
            //     return false;
        }
    };
