...
var Metabiosis = mBss = (function() {
    ...
    function processConfig(cfg) {
        ...
        //     return true;
        // } 
        var p = queueEvents( __paths, __itemKeys );
        if (p) {
            MBS.log("items and actions are processed");
            return true;
        } 
    ...
    function queueEvents(items, key, paths) {
        for (var i = 0; i < items.length; i++) {
            eventQueue = eventQueue.concat( addActionPaths(items[i], key, paths) );
        }
    ...
    function queueHandler(argument) {
